...
const tabs: { id: Tab; label: string; icon: typeof Heart; placeholder: string; description: string }[] = [
  ...
  { id: 'reflexiones', label: 'Reflexiones', icon: MessageCircle, placeholder: '¿Qué te quedó dando vuelta? Comparte tu reflexión...', description: 'Preguntas, pensamientos o algo que resonó en ti.' },
...
export default function ParticipaPage() {
  ...
  function handleReact(id: string) {
    const reacted = localStorage.getItem(`participa_reacted_${id}`);
    if (reacted) return;
    const updated = entries.map(e =>
      e.id === id ? { ...e, reactions: e.reactions + 1 } : e
    );
    saveEntries(updated);
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="flex gap-1 bg-bg-alt rounded-xl p-1 border border-gray-200/50">
        {tabs.map(tab => {
      ...
      <form onSubmit={handleSubmit} className="bg-card rounded-xl p-6 border border-gray-100 shadow-sm space-y-4">
        ...
        <textarea
          ...
          maxLength={500}
          rows={3}
          className="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/30"
          required
        />
      ...
      {filtered.length === 0 ? (
        <p className="text-center text-text-light py-12">
          Aún no hay nada aquí. ¡Sé el primero en participar!
        </p>
      ) : (
        <div className="space-y-3">
          {filtered.map(e => {
            const alreadyReacted = typeof window !== 'undefined' && localStorage.getItem(`participa_reacted_${e.id}`);
            return (
              <div key={e.id} className="bg-card rounded-xl p-5 border border-gray-100 shadow-sm space-y-2">
                ...
                <div className="flex items-center justify-between text-sm text-text-light">
                  ...
                  >
                    <Heart size={14} fill={alreadyReacted ? 'currentColor' : 'none'} /> {e.reactions}
                  </button>
                </div>
              </div>
            );
