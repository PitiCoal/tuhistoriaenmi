...
import { Camera, LogIn, ArrowRight, Heart, Users, MessageCircle, Grid3X3, Mic, Quote, User } from 'lucide-react';
import Link from 'next/link';

const hubLinks = [
  { href: '/participa#oraciones', icon: Heart, label: 'Oraciones', desc: 'Deja tus intenciones' },
  { href: '/participa#reflexiones', icon: Quote, label: 'Reflexiones', desc: 'Comparte pensamientos' },
  { href: '/participa#sugerencias', icon: Mic, label: 'Sugerencias', desc: 'Propón invitados' },
  { href: '/participa#muro', icon: Grid3X3, label: 'Muro', desc: 'Postea a la comunidad' },
];
  ...
  return (
    <div className="space-y-4 md:space-y-6 max-w-2xl mx-auto">
      {!user ? (
        ...
        <>
          <div className="bg-card rounded-xl md:rounded-2xl p-6 md:p-8 border border-primary/20 shadow-md text-center space-y-3 bg-gradient-to-br from-primary/[0.02] to-primary/[0.06]">
            <div className="w-14 h-14 rounded-full bg-primary flex items-center justify-center mx-auto shadow-md">
              <Users size={24} className="text-white" />
            </div>
            <h1 className="font-heading text-xl md:text-2xl font-bold text-primary-dark">Comunidad</h1>
            <p className="text-sm text-text-light">Bienvenido de vuelta. Aquí está todo lo que puedes hacer.</p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            {hubLinks.map(link => (
              <Link key={link.href} href={link.href}
                className="bg-card rounded-xl p-4 md:p-5 border border-gray-200/70 shadow-sm hover:shadow-md transition-shadow text-center space-y-2 hover:border-primary/30"
              >
                <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center mx-auto">
                  <link.icon size={18} className="text-primary" />
                </div>
                <h3 className="font-heading font-bold text-primary-dark text-sm">{link.label}</h3>
                <p className="text-[11px] text-text-light">{link.desc}</p>
              </Link>
            ))}
          </div>
          ...
          <div className="flex items-center justify-center">
            <Link href="/participa"
              className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-primary text-white rounded-lg text-sm font-semibold hover:bg-primary/90 transition-colors shadow-sm">
              Ver todo en Participa <ArrowRight size={14} />
            </Link>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <Link href="/perfil"
              className="bg-card rounded-xl p-4 border border-gray-200/70 shadow-sm hover:shadow-md transition-shadow space-y-2">
              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                <User size={18} className="text-primary" />
              </div>
              <h3 className="font-heading font-bold text-primary-dark text-sm">Mi Perfil</h3>
              <p className="text-[11px] text-text-light">Foto, país, edad, bio</p>
            </Link>
            <a href="https://chat.whatsapp.com/HlF62d1pyiD3Ac98Oe2EKH" target="_blank" rel="noopener noreferrer"
              ...
              <p className="text-[11px] text-text-light">Grupo de la comunidad</p>
            </a>
          </div>

          <div className="bg-card rounded-xl overflow-hidden border border-gray-200/70 shadow-md">
            <div className="flex flex-col sm:flex-row">
              <div className="sm:w-40 h-32 bg-gradient-to-br from-[#833AB4] via-[#FD1D1D] to-[#FCAF45] flex items-center justify-center">
              ...
              <div className="flex-1 p-4 flex items-center justify-between gap-3">
                ...
                <a href="https://instagram.com/tuhistoria.enmi" target="_blank" rel="noopener noreferrer"
                  className="shrink-0 inline-flex items-center gap-1 px-3 py-1.5 bg-instagram text-white rounded-lg text-xs font-semibold hover:opacity-90">
                  Seguir
                </a>
              </div>
            </div>
